...
public class UserServiceImpl implements UserService {
    ...
    public ResultVO checkLogin(String name, String pwd) {
        ...
        if(users.size() == 0){
            return new ResultVO(10000,"登陆失败,用户名不存在！",null);
        }else{
            ...
            if(md5Pwd.equals(users.get(0).getPassword())){
                ...
                String token = builder.setSubject(name)  //主题，就是token中携带的数据
                        ...
                        .setClaims(map) //map中可以存放用户的权限角色信息
                        .setExpiration(new Date(System.currentTimeMillis() + 24 * 60 * 60 * 1000*10))  //设置token的过期时间
